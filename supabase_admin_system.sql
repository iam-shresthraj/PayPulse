...
-- ----------------------------------------------------------------------------
-- 11. Create notifications table and setup RLS policies
-- ----------------------------------------------------------------------------
...
-- Select Policy: Users can view notifications meant for their company or role, or global notifications
DROP POLICY IF EXISTS notifications_select ON public.notifications;
CREATE POLICY notifications_select ON public.notifications
  FOR SELECT USING (
    (target_company_id IS NULL OR target_company_id = public.current_company_id())
    AND (target_role = 'ALL' OR target_role = public.current_role_pp())
  );

-- Super admin can perform all operations
DROP POLICY IF EXISTS notifications_super_all ON public.notifications;
CREATE POLICY notifications_super_all ON public.notifications
